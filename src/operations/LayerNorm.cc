...
LayerNorm::LayerNorm(std::string name, std::vector<Ptr<NPUTensor>> weights)
    : Operation(name) {
...
LayerNorm::get_outputs(std::vector<Ptr<BTensor>> inputs) {
  ...
  for (auto weight_dim_riter = _weight_dim.rbegin();
       weight_dim_riter != _weight_dim.rend(); ++weight_dim_riter) {
    assert((*weight_dim_riter) == (*input_dim_riter));
    assert(input_dim_riter != input_dims.rend());
    input_dim_riter++;
  }
  _outputs[0] = std::make_shared<NPUTensor>(_name + "_output", input_dims,
                                            NPUTensorBufType::ACT, false);

  calculate_loops();
  initialize_tiles();

  spdlog::info("input dims : {} {} {}", _inputs[0]->get_dims(),
               _inputs[1]->get_dims(), _inputs[2]->get_dims());
...
Tile LayerNorm::initialize_instructions(uint32_t N) {
  ...
  addr_type sram_activation_base =
      sram_beta_base + weight_count * _config.precision;
  ...
  for (uint32_t n_inner_offset = 0; n_inner_offset < n_inner;
    ...
    std::vector<addr_type> activation_addrs =
        activation_tensor->get_row_addrs(row_idx);
    ...
    tile.instructions.push_back(Instruction{
        ...
        .src_addrs = std::vector<addr_type>{sram_activation_offset,
                                            sram_gamma_base, sram_beta_base},
    });
    // -- save outputs --
  ...
  // tile.instructions.size(), tile.optype); spdlog::info("outer loop {}, inner
  // loop {}", _outer_loop, _inner_loop);
...
void LayerNorm::calculate_loops() {
  ...
  //              (float)sram_size_needed() / (float)(_config.spad_size KB /
  //              2));
}
