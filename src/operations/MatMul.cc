...
MatMul::get_outputs(std::vector<Ptr<BTensor>> inputs) {
    ...
    for (uint32_t M = 0; M < _outer_loop[0]; ++M) {
      for (uint32_t N = 0; N < _outer_loop[2]; ++N) {
        for (uint32_t K = 0; K < _outer_loop[1]; ++K) {
          _tiles.push_back(
              initialize_instructions(B, M, K, N, K + 1 == _outer_loop[1]));
        }
      }
    }
  }
  ...
  if (_inputs.size() == 3 && K == 0) {
    ...
    for (uint32_t n_inner_offset = 0; n_inner_offset < n_inner;
      ...
      for (uint32_t n_loop = 0; n_loop < loop_size; ++n_loop) {
        ...
        auto bias_addr =
            bias_tensor->get_addr({n_outer_offset + n_inner_offset + n_loop});
      ...
      if (bias_addrs.size() == 0) {
        ...
        tile.instructions.push_back(Instruction{
            .opcode = Opcode::MOVIN,
            .dest_addr =
                sram_accumulation_base + n_inner_offset * _config.precision,
  ...
  for (uint32_t n_inner_offset = 0; n_inner_offset < n_inner;
    ...
    for (uint32_t k_inner_offset = 0; k_inner_offset < k_inner;
      ...
      for (uint32_t m_inner_offset = 0; m_inner_offset < m_inner;
        ...
        if (n_inner_offset == 0) {
          ...
          for (int m_loop = 0; m_loop < loop_size; m_loop++) {
            for (int k_loop = 0; k_loop < loop_size; k_loop++) {
              ...
              activation_indexes.push_back(m_outer_offset + m_inner_offset +
                                           m_loop);
              activation_indexes.push_back(k_outer_offset + k_inner_offset +
                                           k_loop);
          ...
          if (activation_addrs.size() == 0) {
            spdlog::info("zero load for activation m: {} {} / k: "
        ...
        if (m_inner_offset == 0) {
          ...
          for (int k_loop = 0; k_loop < loop_size; k_loop++) {
            for (int n_loop = 0; n_loop < loop_size; n_loop++) {
              ...
              weight_indexes.push_back(k_outer_offset + k_inner_offset +
                                       k_loop);
              weight_indexes.push_back(n_outer_offset + n_inner_offset +
                                       n_loop);
          ...
          if (weight_addrs.size() == 0) {
            ...
            spdlog::info(
                "inner loop {}, outer loop {}, act_size {}, wgt_size {}",
        ...
        // std::cout << "tile " << tile_m << " " << tile_n << " " << tile_k <<
        // std::endl;
        // -- compute --
        // in case of 1st L1 tile execution, execute GEMM_PRELOAD instruction
        tile.instructions.push_back(Instruction{
            .opcode =
                (m_inner_offset == 0 ? Opcode::GEMM_PRELOAD : Opcode::GEMM),
        ...
        if (should_store && (k_inner_offset + loop_size >= k_inner)) {
          ...
          for (int n_loop = 0; n_loop < loop_size; n_loop++) {
            for (int m_loop = 0; m_loop < loop_size; m_loop++) {
              ...
              output_indexes.push_back(m_outer_offset + m_inner_offset +
                                       m_loop);
              output_indexes.push_back(n_outer_offset + n_inner_offset +
                                       n_loop);
...
void MatMul::calculate_loops() {
  ...
  // Calculate the number of iterations for matmul by multiplying all dimensions
  // except the last two.
  _prod_batches = 1;
  auto larger_dims =
      input0_dims.size() > input1_dims.size() ? input0_dims : input1_dims;
