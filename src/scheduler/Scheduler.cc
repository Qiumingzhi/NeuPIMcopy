...
Scheduler::Scheduler(SimulationConfig config, const cycle_type *core_cycle)
  ...
  // 对应 Systolic Array (NPU) 的执行程序
  _model_program2 = nullptr;
  // 对应 PIM 的执行程序

  _init_stage = Stage::A; // 初始阶段设为 A (通常是 QKV 生成)
  // _init_stage = Stage::C;
  _stage = _init_stage;    // 当前阶段
  _just_one_stage = false; // 调试用标志：是否只运行一个阶段

  _has_stage_changed = false;

  _partition_alg_simple =
      true; // 默认使用简单的对半切分算法进行子批次划分 false使用动态规划的方法

  // Request queue for channel
  for (int i = 0; i < _dram_channels; i++) {
    auto req_q = std::vector<Ptr<InferRequest>>(); //存放请求
    auto req_latency_q = std::vector<uint32_t>();
    req_q.reserve(_max_batch_size); //预分配空间
...
void Scheduler::allocate_requests() {
  ...
  for (auto it = _request_queue.begin(); it != _request_queue.end();
    ...
    if (!request->is_initiated) { //判断该请求是否是第一次被调度器处理
      int ch =
          request
              ->channel; //调用之前python脚本已经生成的请求trace文件里面分配的channel
      ...
      if (_active_reqs >= _max_active_reqs)
        continue;
      _active_reqs++; //限制并发请求数量 目前并发请求=max_batch_size
      // spdlog::info("Scheduler allocate request#{}(seq_len:{}) to channel
      // {}<<",
      //              request->id, seq_len, ch);

      // 定义 Key 和 Value 张量的维度
      auto k = std::make_shared<PIMTensor>(
          name_gen(std::to_string(request->id), "KEY", std::to_string(0)), ch,
          dim_key, PIMTensorKVType::KEY, true);
      auto v = std::make_shared<PIMTensor>(
          name_gen(std::to_string(request->id), "VALUE", std::to_string(0)), ch,
          dim_value, PIMTensorKVType::VALUE, true);
      request->K_cache.push_back(k); // 将张量挂载到请求对象上
      request->V_cache.push_back(v);
      //这里并没有真正分配物理内存（那是
      // Dram类的工作），而是创建了逻辑上的张量对象，用于后续生成指令和跟踪依赖关系。

      // 5. 注册到通道队列与延迟估算
      _active_request_queues[ch].push_back(
          request); // 将请求加入到对应通道的活跃队列中

      // 估算该请求在 PIM 上执行 MHA (Multi-Head Attention) 需要的延迟
...
void Scheduler::make_program() {
  ...
  _model_program2 = std::make_unique<StageProgram>(_model, sub_batch_on_pim,
                                                   StagePlatform::PIM, _stage);
...
void Scheduler::group_sub_batches() {
  if (!_config.sub_batch_mode) { //没有用子批次时候
    //>>>
  ...
  for (int ch = 0; ch < _dram_channels; ch++) {
    ...
    if (_partition_alg_simple) {
      ...
      spdlog::info("Simple Split: Total={}, B1={}, B2={}", req_queue.size(),
                   _breq1.size(), _breq2.size());

    } else {
      //它的作用当你把 _partition_alg_simple 设为 false
      //后，模拟器在运行时，对于每一个Channel 的 RequestQueue，
      //它不再是无脑对半切，而是会计算每个 Request 的预计 MHA
      //延迟，然后用背包问题（Knapsack-like）的思路把这些 Request 分配到 Batch 1
      //和 Batch 2，使得：
      // $$ \sum Latency(Batch_1) \approx \sum Latency(Batch_2) $$
      //这实际上是在解决 MHA 阶段内部的负载均衡。

      //注意：它平衡的是 Batch 1 的 MHA 总时间 vs Batch 2 的 MHA 总时间。
      //它并没有显式地去平衡 SA (Stage C) 和 PIM (Stage C)
      //之间的不平衡（那是我们之前讨论的那个长短序列问题），那是另一个维度的优化。但如果你的
      // Request 里有些长有些短，DP 切分能防止“长序列都挤在 Batch
      // 1，短序列都挤在 Batch 2”的情况，从而避免因 MHA
      // 执行时间波动导致的严重气泡。

      // 目前仿真得到的总时钟周期是1,111,173 效果反而不太好
      ...
      // spdlog::info("#1 sum:{:2d}, idx:[{}], time:[{}]", sum_list1_latencies,
      // list1_str,
      //              time1_str);
      // spdlog::info("#2 sum:{:2d}, idx:[{}], time:[{}]", sum_list2_latencies,
      // list2_str,
      //              time2_str);
...
bool Scheduler::finish_tile(uint32_t core_id, Tile &tile) {
  bool result = false;
  spdlog::debug("Tile {} Core {} Finish Tile at {}", tile.operation_id, core_id,
                *_core_cycle);
  ...
  if (_active_operation_stats[tile.operation_id].remain_tiles == 0) {
    ...
    spdlog::info("Layer {} finish at {}",
                 _active_operation_stats[tile.operation_id].name, *_core_cycle);
...
Scheduler::partition_lists_simple(std::vector<uint32_t> originalVector) {
...
std::pair<std::vector<int>, std::vector<int>>
Scheduler::partition_lists_dp(std::vector<uint32_t> inputList) {
  //这段代码实现了一个经典的子集和问题 (Subset Sum Problem)
  //的动态规划解法，用于将一组请求尽可能平均地分成两组，使得两组的总延迟（Latency）最接近。

  //算法目标
  //给定一个整数列表 inputList（代表每个请求的延迟），将其划分为两个子集 list1
  //和 list2，使得 sum(list1) 尽可能接近 totalSum / 2。
  int totalSum = 0;
