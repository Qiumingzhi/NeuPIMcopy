...
Model::Model(SimulationConfig config, std::string name) {
  ...
  _num_batch = 3;
  _num_token = 13;
  _target_token = 14; //这几个值都是仅作测试遗留下来的
...
/*
apply parallelism
pipeline parallelism in attention is applied to column(QKVgen, fc1) and
row(proj, fc2) need for layernorm variable to be at all chip
*/
void Model::init_params() {
  // 根据配置文件初始化整个模型的所有权重参数（Tensor）
  for (int i = 0; i < _config.model_n_layer; ++i) {
    auto attn = name_gen(LAYER(i), BlockType::Attention);
    create_weight(
        name_gen(attn, OperationType::LayerNorm, ParameterType::Weight),
        {_config.model_n_embd});
    create_weight(name_gen(attn, OperationType::LayerNorm, ParameterType::Bias),
                  {_config.model_n_embd});
    create_weight(
        name_gen(attn, OperationType::QKVGen, ParameterType::Weight),
    ...
    create_weight(
        name_gen(attn, OperationType::Projection, ParameterType::Weight),
        {_config.model_n_embd / _config.n_tp, _config.model_n_embd});
    create_weight(
        name_gen(attn, OperationType::Projection, ParameterType::Bias),
        {_config.model_n_embd});

    auto ffn = name_gen(LAYER(i), BlockType::FeedForward);
    create_weight(
        name_gen(ffn, OperationType::LayerNorm, ParameterType::Weight),
        {_config.model_n_embd});
    create_weight(name_gen(ffn, OperationType::LayerNorm, ParameterType::Bias),
                  {_config.model_n_embd});
    create_weight(
        name_gen(ffn, OperationType::FullyConnected1, ParameterType::Weight),
        {_config.model_n_embd, 4 * _config.model_n_embd / _config.n_tp});
    create_weight(
        name_gen(ffn, OperationType::FullyConnected1, ParameterType::Bias),
        {4 * _config.model_n_embd / _config.n_tp});
    create_weight(
        name_gen(ffn, OperationType::FullyConnected2, ParameterType::Weight),
        {4 * _config.model_n_embd / _config.n_tp, _config.model_n_embd});
    create_weight(
        name_gen(ffn, OperationType::FullyConnected2, ParameterType::Bias),
        {_config.model_n_embd});
  }
...
std::vector<Ptr<NPUTensor>> Model::get_params(int layer_idx,
...
std::shared_ptr<Tensor> Model::load_cache(uint32_t layer, std::string type) {
  ...
  if (type == "key") {
    shape.assign({_config.model_n_head,
                  _config.model_n_embd / _config.model_n_head, _num_token});
  } else if (type == "value") {
    shape.assign({_config.model_n_head, _num_token,
                  _config.model_n_embd / _config.model_n_head});
  }
...
Model::register_operation(std::shared_ptr<Operation> op) {
  // auto id = op->get_id();
...
// std::shared_ptr<Tensor> Model::get_tensor(uint32_t id) { return
// _tensor_map[id]; }
...
std::shared_ptr<Tensor> Model::create_tensor(std::string name,
...
std::shared_ptr<NPUTensor> Model::create_weight(std::string name,
                                                std::vector<uint32_t> dims) {
  // create_tensor(name, dims, TensorBufType::WGT);
  // auto tensor = std::make_shared<BatchedTensor>(name, dims,
  // TensorBufType::WGT, true);
  auto tensor =
      std::make_shared<NPUTensor>(name, dims, NPUTensorBufType::WGT, true);
...
//     }
//     log_file <<
//     "op_name\tstart_cycle\tend_cycle\tcompute_cycles\tnum_calculation\tmemory_"
