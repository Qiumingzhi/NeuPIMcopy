...
uint32_t generate_id() {
  ...
  // 静态局部变量 初值为0 作用是给某个对象创建一个独一无二的ID
  return id_counter++;
}
...
void initialize_client_config(std::string cli_config_path) {
  ...
  // cli_config["request_input_seq_len"]; Config::global_config.request_interval
  // = cli_config["request_interval"]; Config::global_config.request_total_cnt =
