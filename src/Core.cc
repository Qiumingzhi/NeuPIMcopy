...
Core::Core(uint32_t id, SimulationConfig config)
    : _id(id), _config(config), _core_cycle(0), _compute_end_cycle(0),
      _stat_idle_cycle(0), _stat_compute_cycle(0), _stat_memory_cycle(0),
      _stat_vec_compute_cycle(0), _stat_vec_memory_cycle(0),
      _memory_stall_cycle(0), _compute_memory_stall_cycle(0),
      _vector_memory_stall_cycle(0), _layernorm_stall_cycle(0),
      _softmax_stall_cycle(0), _add_stall_cycle(0), _gelu_stall_cycle(0),
      _load_memory_cycle(0), _store_memory_cycle(0), _stat_vec_idle_cycle(0),
      _stat_matmul_cycle(0), _stat_layernorm_cycle(0), _stat_add_cycle(0),
      _stat_gelu_cycle(0), _stat_softmax_cycle(0),
    ...
    for (auto tile : _tiles) {
      if (tile->accum_spad_id == next_acc_spad) {
        if (tile->remaining_accum_io != 0) {
          ...
          // tile->accum_spad_id); std::cout << tile->remaining_accum_io <<
          // std::endl;
          return false;
        }
      }
...
// 将一个新的计算任务块（Tile）发射（Issue）到 NPU
// 核心中进行处理。Tile的定义在Common.h中
...
void Core::print_stats() {
  ...
  spdlog::info("Core [{}] : MatMul stall cycle {} LayerNorm stall cycle {} "
  ...
  spdlog::info("Core [{}] : Load stall cycle {} Store stall cycle {} "
