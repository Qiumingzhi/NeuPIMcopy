...
// TODO: num_cycles is a magic number. it counts memory load store for 50
// core-cycles
// TODO: convert global cycle to core cycle
//  global time / core freq -> core cycle
// memory_reads, memory_writes: bytes
typedef struct NPUStat { // NPU 核心状态
  NPUStat() = default;
  NPUStat(uint64_t core_cycle_)
      : start_cycle(core_cycle_), num_calculations(0) {}

  uint64_t start_cycle; // 记录开始时间和持续时长。
  uint64_t num_cycles;
  uint64_t num_calculations; // num_calculations:
                             // 记录总共执行了多少次乘加运算（MAC operations）。
...
typedef struct MemoryIOStat {
  ...
  MemoryIOStat(uint64_t core_cycle_, uint64_t channel_id_, uint64_t num_cycles_)
      : start_cycle(core_cycle_), channel_id(channel_id_),
        num_cycles(num_cycles_), memory_reads(0), memory_writes(0),
        pim_reads(0) {}
...
typedef struct TileStat {
  ...
  TileStat(uint64_t core_cycle)
      : start_cycle(core_cycle), end_cycle(0), compute_cycles(0),
        weight_load_cycles(0), memory_stalls(0), systolic_memory_stalls(0),
        memory_reads(0), memory_writes(0), sram_reads(0), sram_writes(0),
        num_calculation(0) {}
...
typedef struct OperationStat {
  ...
  OperationStat(std::string name)
      : op_name(name), start_cycle(-1), end_cycle(0), compute_cycles(0),
        weight_load_cycles(0), memory_stalls(0), systolic_memory_stalls(0),
        memory_reads(0), memory_writes(0), sram_reads(0), sram_writes(0),
        num_calculation(0) {}
  ...
  void update_stat(TileStat tile_stat) {
    ...
    end_cycle =
        end_cycle > tile_stat.end_cycle ? end_cycle : tile_stat.end_cycle;
  ...
  std::string get_by_enum(StatType stat_type) {
    ...
    case StatType::TotalMemoryBandwidth:
      return std::to_string((memory_reads + memory_writes) * core_cycle /
                            total_cycle);
    case StatType::NumCalculation:
