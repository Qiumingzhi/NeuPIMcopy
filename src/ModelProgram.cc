...
//注意： 在Simulator仿真器中并没有使用

ModelProgram::ModelProgram(Ptr<Model> model,
...
void ModelProgram::init_program() {
  ...
  if (end_to_end) {
    ...
    for (uint32_t layer_idx = 0;
         layer_idx < Config::global_config.model_n_layer; ++layer_idx) {
      if (npu_program) {
        if (fused)
  ...
  /* only Multi-head attention layer */
    ...
    spdlog::info("*** K_cache.size(num_layers): {}",
                 _breq->_reqs[0]->K_cache.size());
    ...
    for (int i = 0; i < Config::global_config.model_n_layer; ++i) {
      ...
      for (int j = 0; j < batch_size; j++) {
        ...
        query = std::make_shared<NPUTensor>(
            "query",
            std::vector<uint32_t>{num_heads, static_cast<unsigned int>(lj), dk},
            NPUTensorBufType::ACT, true);
      ...
      if (npu_program) {
        ...
        auto logit_softmax = add_op(std::make_shared<NeuPIMSLogitSoftmax>(
            name_gen(prefix, BlockType::Attention,
                     OperationType::NeuPIMSLogitSoftmax)));
        inputs = get_outputs(logit_softmax, mha_pim_inputs);

        /* pim_gemv + add */
        inputs.insert(inputs.end(), values.begin(),
                      values.end()); // logits, values

        auto attend = add_op(std::make_shared<NeuPIMSAttend>(name_gen(
            prefix, BlockType::Attention, OperationType::NeuPIMSAttend)));
...
Ptr<Operation> ModelProgram::add_op(std::shared_ptr<Operation> op) {
  // 这是一个智能指针shared_ptr 尖括号里指定了指针指向的数据类型是 Operation
  // 关键点：因为 Operation 是一个基类（父类），根据 C++
  // 的规则，基类的指针可以指向任何它的子类对象。 这里 op 可以是任何继承自
  // Operation 的子类对象，比如 NeuPIMSLogitSoftmax、NeuPIMSAttend 等。
...
ModelProgram::get_outputs(Ptr<Operation> op, std::vector<Ptr<BTensor>> inputs) {
...
std::vector<Ptr<BTensor>>
ModelProgram::fused_pim_attn_block(uint32_t layer,
                                   std::vector<Ptr<BTensor>> inputs) {
  ...
  auto split = add_op(std::make_shared<Split>(
      name_gen(prefix, OperationType::BatchSplit), num_rows_breakdown, 0));
  ...
  for (int request_index = 0; request_index < num_requests; ++request_index) {
    ...
    auto split = add_op(std::make_shared<SplitDecoding>(
        name_gen(prefix, OperationType::QKVSplit,
                 std::to_string(request_index)),
        _breq->get_cache(layer, request_index),
        _breq->is_initiated(request_index)));
  ...
  auto logit_softmax = add_op(std::make_shared<NeuPIMSLogitSoftmax>(name_gen(
      prefix, BlockType::Attention, OperationType::NeuPIMSLogitSoftmax)));
  ...
  for (int request_index = 0; request_index < num_requests; ++request_index) {
    ...
    auto reshape = add_op(
        std::make_shared<Reshape>(name_gen(prefix, OperationType::AReshape,
                                           std::to_string(request_index)),
                                  reshape_result));
  ...
  // collect batches
  // (T1,E),(T2,E), ... ,(Tn,E) -> (N,E)
  auto concat = add_op(std::make_shared<Concat>(
      name_gen(prefix, OperationType::BatchConcat), 0));
...
std::vector<Ptr<BTensor>>
ModelProgram::pim_attn_block(uint32_t layer, std::vector<Ptr<BTensor>> inputs) {
  auto prefix = name_gen(LAYER(layer), BlockType::Attention);
  auto res_buf = inputs[0];
  ...
  auto pim_gemv = add_op(
      std::make_shared<PIMGEMV>(name_gen(prefix, OperationType::PIMGEMV)));
  // auto pim_gemv = block_gemv_softmax(prefix);
  auto l = get_outputs(pim_gemv, querys);

  /* softmax */
  auto logit_softmax = add_op(
      std::make_shared<Softmax>(name_gen(prefix, OperationType::SoftMax)));
 ...
 *  input:
 *      (N,E) 2d matrix.
 *      Note that N is not the number of batch size, but sum of n batches token
 * len. This means, N = T1 + T2 + ... + Tn output: (N,E) 2d matrix.
 */
std::vector<Ptr<BTensor>>
ModelProgram::attn_block(uint32_t layer, std::vector<Ptr<BTensor>> inputs) {
  auto prefix = name_gen(LAYER(layer), BlockType::Attention);
  auto res_buf = inputs[0];
  ...
  for (int request_index = 0; request_index < num_requests; ++request_index) {
    ...
    auto l_mm = add_op(std::make_shared<MatMul>(name_gen(
        prefix, OperationType::QKMatMul, std::to_string(request_index))));
    auto l = get_outputs(
        l_mm, std::vector<Ptr<BTensor>>{qkv_cached[0], qkv_cached[1]});

    auto ls_sm = add_op(std::make_shared<Softmax>(name_gen(
        prefix, OperationType::SoftMax, std::to_string(request_index))));
    auto ls = get_outputs(ls_sm, l);

    // (nh,{1,T},{T+1,T})@(nh,{T+1,T},dk) -> (nh,{1,T},dk)
    auto a_mm = add_op(std::make_shared<MatMul>(name_gen(
        prefix, OperationType::LsVMatMul, std::to_string(request_index))));
 ...
 */
std::vector<Ptr<BTensor>>
ModelProgram::fused_attn_block(uint32_t layer,
                               std::vector<Ptr<BTensor>> inputs) {
  auto prefix = name_gen(LAYER(layer), BlockType::Attention);
  auto res_buf = inputs[0];
  ...
  // (N,E) -> (t1,E),(t2,E), ..., (tn,E)
  // auto num_rows_breakdown = _breq->get_num_rows_breakdown();
  // auto split = add_op(std::make_shared<Split>(name_gen(prefix,
  ...
  // for (int request_index = 0; request_index < _breq->get_num_reqs();
  // ++request_index) {
  //     std::vector<Ptr<BTensor>> qkv(qkv_per_reqs.begin() + request_index,
  //                                   qkv_per_reqs.begin() + request_index +
  //                                   1);
  //     std::vector<Ptr<BTensor>> qkv_cached;
  ...
  //     auto split = add_op(std::make_shared<SplitDecoding>(
  //         name_gen(prefix, OperationType::QKVSplit,
  //         std::to_string(request_index)), _breq->get_cache(layer,
  //         request_index), _breq->is_initiated(request_index)));
  //     qkv_cached = get_outputs(split, qkv);
  ...
  // for (int request_index = 0; request_index < _breq->get_num_reqs();
  // ++request_index) {
  //     std::vector<uint32_t> reshape_result =
  //     {inputs[request_index]->get_dims()[1],
  //                                             Config::global_config.model_n_embd};
  //     auto reshape = add_op(std::make_shared<Reshape>(
  //         name_gen(prefix, OperationType::AReshape,
  //         std::to_string(request_index)), reshape_result));
  ...
  // // collect batches
  // // (T1,E),(T2,E), ... ,(Tn,E) -> (N,E)
  // auto concat = add_op(std::make_shared<Concat>(name_gen(prefix,
  // OperationType::BatchConcat), 0)); inputs = get_outputs(concat,
  // qkv_results);
  ...
  // auto residual = add_op(std::make_shared<Add>(name_gen(prefix,
  // OperationType::Residual))); inputs.push_back(res_buf); inputs =
  // get_outputs(residual, inputs);
 ...
 */
std::vector<Ptr<BTensor>>
ModelProgram::ffn_block(uint32_t layer, std::vector<Ptr<BTensor>> inputs) {
  auto res_buf = inputs[0];
...
Ptr<Operation> ModelProgram::block_QKV_split(uint32_t layer, uint32_t unit,
                                             uint32_t dim_idx) {
  // return add_op(std::make_shared<Split>(
  //     name_gen(LAYER(layer), BlockType::Attention, OperationType::QKVSplit),
  //     unit, dim_idx));
}

Ptr<Operation> ModelProgram::test_block_gelu(uint32_t layer) {
  return add_op(std::make_shared<Gelu>(
      name_gen(LAYER(layer), BlockType::Attention, OperationType::Gelu)));
}
...
//     return add_op(std::make_shared<MatMul>(
//         name_gen(LAYER(layer), BlockType::Attention, OperationType::QKVGen),
//         _model->get_param(layer, BlockType::Attention,
//         OperationType::QKVGen)));
// }

// std::shared_ptr<Operation> ModelProgram::block_QKV_split(uint32_t layer,
// uint32_t unit,
//                                                          uint32_t dim) {
//     return add_op(std::make_shared<Split>(
//         name_gen(LAYER(layer), BlockType::Attention,
//         OperationType::QKVSplit), unit, dim));
// }
