 ...
 *  The size of each allocated block is _kv_cache_entry_size(32) * d_k *
 *  precision Memory layout for Key and Value is (d_k,n) and (n,d_k) each.
...
NPUTensorKV::NPUTensorKV(std::vector<uint32_t> dims, NPUTensorKVType kv_type)
  ...
  uint32_t num_required_alloc =
      ceil((double)_seq_len / (double)_kv_cache_entry_size);
...
addr_type NPUTensorKV::get_addr(std::vector<uint32_t> indexes) {
  ...
  uint32_t byte_idx =
      _kv_type == NPUTensorKVType::KEY ? indexes[0] : indexes[1];
  ...
  uint32_t offset =
      ((seq_idx % _kv_cache_entry_size) * dk + byte_idx) * _precision;
...
uint32_t NPUTensorKV::get_allocated_seq_len() {
  return _bases.size() * _kv_cache_entry_size;
}
